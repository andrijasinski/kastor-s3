...
describe('GET /api/buckets/:bucket/download-folder', () => {
  it('returns 200 with zip content-type and correct filename', async () => {
    ...
    expect(res.headers.get('Content-Disposition')).toBe("attachment; filename*=UTF-8''photos.zip");
  });
  ...
  it('uses bucket name as filename when prefix is empty', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers.get('Content-Disposition')).toBe("attachment; filename*=UTF-8''testBucket.zip");
  });
